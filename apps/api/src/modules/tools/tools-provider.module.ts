...
export class ToolsProviderModule implements OnModuleInit {
  ...
  private registerTools() {
    // ─── Data & Documents ───────────────────────────────────────────
    this.registry.register(ToolAdapter.from({
      name: 'extract_structured_data',
      displayName: 'Ekstraksi Data',
      description: 'Validasi dan normalisasi data terstruktur dari dokumen. Kirim data yang sudah diekstrak, bukan teks mentah.',
      tags: ['extract', 'data', 'validate'],
      handler: (args) => this.textExtractorTool.extractStructuredData({
    ...
    this.registry.register(ToolAdapter.from({
      ...
      handler: async (args) => {
        if (args.action === 'list_tables') return this.dataQueryTool.listTables();
        if (args.action === 'describe_table' && args.tableName) return this.dataQueryTool.describeTable(args.tableName);
        return this.dataQueryTool.queryData(args.sql || '');
      },
      parameters: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['query', 'list_tables', 'describe_table'], description: 'Aksi: query (jalankan SQL), list_tables, describe_table' },
          sql: { type: 'string', description: 'SQL SELECT query' },
          tableName: { type: 'string', description: 'Nama table (untuk describe_table)' },
        },
        required: ['action'],
      },
      timeoutMs: 10000,
    ...
    this.registry.register(ToolAdapter.from({
      ...
      handler: (args) => this.imageOcrTool.recognizeText(args.filePath, args.language),
      parameters: {
        type: 'object',
        properties: {
          filePath: { type: 'string', description: 'Path ke file gambar' },
          language: { type: 'string', description: 'Bahasa OCR (default: eng)' },
        },
        required: ['filePath'],
      },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      handler: (args) => this.docSearchTool.searchDocuments(args.query, args.limit),
      parameters: {
        type: 'object',
        properties: {
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Melakukan kalkulasi numerik — subtotal, pajak, diskon, total, atau operasi matematika apapun.',
      tags: ['calculate', 'math', 'finance', 'tax', 'discount', 'total'],
      handler: (args) => this.calculatorTool.calculateFinancials(
        args.items || [],
        args.taxPercent || 0,
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Mengonversi data terstruktur menjadi file siap download — Excel (xlsx), CSV, PDF, Word (docx), atau PowerPoint (pptx).',
      tags: ['export', 'document', 'pdf', 'docx', 'pptx', 'xlsx', 'csv', 'spreadsheet', 'presentation', 'invoice', 'report'],
      handler: (args) => this.handleGenerateExport(args),
      parameters: {
        type: 'object',
        properties: {
          format: { type: 'string', enum: ['xlsx', 'csv', 'pdf', 'docx', 'pptx'], description: 'Format output file' },
          title: { type: 'string', description: 'Judul dokumen' },
          content: { type: 'string', description: 'Isi dokumen dalam bentuk teks/markdown (untuk pdf, docx)' },
          sheetName: { type: 'string', description: 'Nama sheet (untuk xlsx/csv)' },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Menyimpan atau memperbarui Knowledge Base. Gunakan saat user ingin membuat atau mengupdate knowledge.',
      tags: ['knowledge', 'save', 'create', 'update', 'base'],
      handler: (args) => this.knowledgeBuilderTool.saveKnowledge(args.title, args.content, args.type),
      parameters: {
        type: 'object',
        properties: {
          title: { type: 'string', description: 'Judul knowledge (nama bisnis/perusahaan)' },
          content: { type: 'string', description: 'Isi knowledge dalam format markdown' },
          type: { type: 'string', description: 'Tipe knowledge berdasarkan domain bisnis' },
        },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Mencari informasi real-time di internet (harga bahan, berita pasar, kurs, kompetitor, dll).',
      tags: ['search', 'web', 'internet', 'realtime', 'google', 'tavily'],
      handler: (args) => this.webSearchTool.searchWeb(args.query, args.searchDepth),
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Kata kunci atau pertanyaan yang ingin dicari di internet' },
          searchDepth: { type: 'string', enum: ['basic', 'advanced'], description: 'Kedalaman pencarian: basic (cepat) atau advanced (mendalam)' },
        },
        required: ['query'],
      },
      estimatedLatency: 'medium',
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Menganalisis gambar/foto (struk belanja, nota lecek, kwitansi, tulisan tangan, gambar produk) menggunakan Vision AI.',
      tags: ['vision', 'ocr', 'image', 'receipt', 'nota', 'foto', 'struk'],
      handler: (args) => this.visionAiTool.analyzeImage(args.imageSource, args.prompt),
      parameters: {
        type: 'object',
        properties: {
          imageSource: { type: 'string', description: 'Path file gambar lokal atau URL gambar' },
          prompt: { type: 'string', description: 'Instruksi spesifik apa yang ingin diekstrak dari gambar' },
        },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Mengonversi nilai antara berbagai satuan (yard, meter, cm, roll, kg, gram, lusin, kodi) atau mata uang (usd, idr, eur, sgd).',
      tags: ['converter', 'unit', 'currency', 'domain-config'],
      handler: (args) => this.unitConverterTool.convert({
      ...
      parameters: {
        ...
        properties: {
          ...
          domain: { type: 'string', description: 'Tipe bisnis untuk unit spesifik' },
        },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Membuat draf pesan profesional untuk WhatsApp, Email formal, Penawaran Harga (Quotation), atau Pengingat Tagihan (Invoice Reminder).',
      tags: ['draft', 'whatsapp', 'email', 'quotation', 'invoice', 'communication'],
      handler: (args) => this.draftCommunicationTool.draft({
        type: args.type,
      ...
      parameters: {
        type: 'object',
        properties: {
          type: { type: 'string', enum: ['whatsapp', 'email', 'quotation', 'invoice_reminder'], description: 'Jenis draf komunikasi' },
          recipientName: { type: 'string', description: 'Nama penerima / klien' },
          topic: { type: 'string', description: 'Topik atau perihal pesan' },
          keyPoints: { type: 'array', items: { type: 'string' }, description: 'Poin-poin penting yang ingin disampaikan' },
        },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      parameters: {
        type: 'object',
        properties: {
          filePath: { type: 'string', description: 'Nama file atau path lengkap file dokumen' },
          workspaceId: { type: 'string', description: 'ID Workspace' },
        },
        required: ['filePath'],
      },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      handler: (args) => this.workspaceToolsService.writeWorkspaceFile({
      ...
      parameters: {
        type: 'object',
        properties: {
          filename: { type: 'string', description: 'Nama file yang akan dibuat' },
          format: { type: 'string', enum: ['xlsx', 'csv', 'pdf', 'docx', 'txt', 'md', 'json'], description: 'Format dokumen' },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string', description: 'Nama skill dalam snake_case' },
          displayName: { type: 'string', description: 'Nama tampilan skill' },
          description: { type: 'string', description: 'Deskripsi singkat skill' },
          category: { type: 'string', enum: ['general', 'data-processing', 'reporting', 'integration'], description: 'Kategori skill' },
          content: { type: 'string', description: 'Instruksi lengkap skill dalam format markdown' },
          tags: { type: 'array', items: { type: 'string' }, description: 'Tag untuk pencarian' },
        },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      handler: (args) => this.skillsTool.updateSkill(args.name, {
        displayName: args.displayName,
        description: args.description,
        content: args.content,
        tags: args.tags,
      ...
      parameters: {
        ...
        properties: {
          ...
          tags: { type: 'array', items: { type: 'string' }, description: 'Tags baru' },
        },
        required: ['name'],
      },
      timeoutMs: 5000,
    ...
    this.registry.register(ToolAdapter.from({
      ...
      parameters: {
        type: 'object',
        properties: {
          type: { type: 'string', enum: ['preference', 'business_fact', 'correction', 'workspace_history', 'context', 'interaction'], description: 'Jenis memory' },
          key: { type: 'string', description: 'Kunci unik memory' },
          content: { type: 'string', description: 'Isi memory' },
          importance: { type: 'number', description: 'Tingkat kepentingan 1-10' },
    ...
    this.registry.register(ToolAdapter.from({
      ...
      description: 'Search across all past conversations for relevant context (FTS5 full-text search).',
      ...
      parameters: {
        ...
        properties: {
          ...
          workspaceId: { type: 'string', description: 'Optional workspace ID to limit search scope' },
        },
        required: ['query'],
      },
      timeoutMs: 5000,
  ...
  private async handleGenerateExport(args: Record<string, any>): Promise<any> {
    ...
    switch (format) {
      case 'csv':
        return this.documentGeneratorTool.generateCsv(args.rows || [], filename);
      case 'pdf':
        return this.documentGeneratorTool.generatePdf(args.title || 'Dokumen', args.content || '', filename);
      case 'docx':
        return this.documentGeneratorTool.generateDocx(args.title || 'Dokumen', args.content || '', filename);
      case 'pptx':
        return this.documentGeneratorTool.generatePptx(args.title || 'Presentasi', args.slides || [{ content: args.content || '' }], filename);
      case 'xlsx':
      default:
        return this.documentGeneratorTool.generateExcel(args.sheetName || 'Data', args.rows || [], filename);
    }
  }
}
