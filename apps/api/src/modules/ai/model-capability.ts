...
export function normalizeModelName(modelName: string): string {
  if (!modelName) return '';
  const withoutSuffix = modelName.replace(
    /:(free|extended|paid|nitro|online|exact)$/i,
    '',
  );
...
export function getModelCapability(modelName: string): ModelCapability {
  ...
  else if (lower.includes('16k') || lower.includes('32k'))
    contextWindow = 32000;
