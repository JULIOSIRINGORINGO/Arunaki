...
describe('TrajectoryAuditService — Execution Audit Trail', () => {
  ...
  it('should record steps and retrieve trajectory by runId', () => {
    ...
    auditService.recordStep(runId, chatId, 'agent_start', { userContent: 'Calculate total revenue' });
    auditService.recordStep(runId, chatId, 'thinking', { message: 'Reading data' });
  ...
  it('should export structured JSON compliance audit report', () => {
    ...
    auditService.recordStep(runId, chatId, 'agent_start', { userContent: 'Baca 3 PDF' });
    auditService.recordStep(runId, chatId, 'sub_agent_spawn', { taskName: 'PDF 1' });
    auditService.recordStep(runId, chatId, 'tool_done', { toolName: 'read_document_content' });
    auditService.recordStep(runId, chatId, 'self_heal', { strategy: 'arg_repair' });
  ...
  it('should handle failed run trajectory export', () => {
    ...
    auditService.recordStep(runId, chatId, 'agent_start', { userContent: 'Proses data' });
    auditService.recordStep(runId, chatId, 'agent_error', { error: 'Network timeout' });
