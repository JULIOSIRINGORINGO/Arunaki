...
describe("Test Models Catalog", () => {
  it("should test models", async () => {
    ...
    await prisma.provider.update({ where: { id: provider.id }, data: { active: true, priority: 1 } });
    ...
    for (const model of MODELS_TO_TEST) {
      ...
      const prompt = 'Sebutkan 1 file apa saja yang ada di direktori kerja saya saat ini (gunakan tools untuk melihat file). Jawab dengan sangat singkat.';
