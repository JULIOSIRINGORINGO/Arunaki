...
export class PromptInjectionDetector {
  ...
  private readonly injectionPatterns = [
    ...
    {
      pattern:
        /you\s+are\s+(?:now\s+)?(?:a\s+)?(?:hacker|admin|root|developer|system|god)/i,
      severity: 'high',
      type: 'role_override',
    },
    {
      pattern:
        /act\s+as\s+(?:a\s+)?(?:hacker|admin|root|developer|system|god)/i,
      severity: 'high',
      type: 'role_override',
    },
    {
      pattern:
        /pretend\s+(?:to\s+be\s+)?(?:a\s+)?(?:hacker|admin|root|developer|system|god)/i,
      severity: 'high',
      type: 'role_override',
    },
    {
      pattern:
        /simulate\s+(?:a\s+)?(?:hacker|admin|root|developer|system|god)/i,
      severity: 'high',
      type: 'role_override',
    },
    ...
    {
      pattern:
        /(?:show|reveal|print|output|display)\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions?|rules?)/i,
      severity: 'high',
    ...
    {
      pattern:
        /(?:call|invoke|execute|run)\s+(?:function|tool)\s+(?:without|bypass|skip)/i,
      severity: 'high',
      type: 'tool_manipulation',
    },
    {
      pattern: /bypass\s+(?:security|safety|validation|checks?)/i,
      severity: 'high',
      type: 'tool_manipulation',
    },
    ...
    {
      pattern:
        /(?:send|post|upload|exfiltrate)\s+(?:data|info|secrets?|keys?|tokens?)/i,
      severity: 'high',
      type: 'data_exfiltration',
    },
    {
      pattern:
        /(?:what|show)\s+(?:is\s+)?(?:your\s+)?(?:api\s+key|secret|password|token)/i,
      severity: 'high',
      type: 'data_exfiltration',
    },
    ...
    {
      pattern:
        /(?:show|reveal|print)\s+(?:your\s+)?(?:reasoning|thinking|chain\s+of\s+thought)/i,
      severity: 'medium',
      type: 'cot_extraction',
    },
    {
      pattern: /what\s+(?:were\s+)?(?:you\s+)?thinking/i,
      severity: 'low',
      type: 'cot_extraction',
    },
    ...
    {
      pattern: /(?:base64|rot13|hex|url\s*encod)/i,
      severity: 'medium',
      type: 'obfuscation',
    },
    ...
    {
      pattern:
        /(?:please|urgent|emergency|help\s+me)\s+.*(?:ignore|bypass|override)/i,
      severity: 'medium',
      type: 'emotional_manipulation',
    },
    {
      pattern:
        /(?:my\s+(?:grandma|mother|father|child|dog)\s+(?:died|is\s+dying|will\s+die))/i,
      severity: 'medium',
      type: 'emotional_manipulation',
    },
    ...
    {
      pattern:
        /continue\s+(?:the\s+)?(?:above|previous)\s+(?:text|response|output)/i,
      severity: 'low',
      type: 'continuation',
    },
    {
      pattern:
        /complete\s+(?:the\s+)?(?:above|previous)\s+(?:text|response|output)/i,
      severity: 'low',
      type: 'continuation',
    },
  ];
  ...
  private readonly sanitizationPatterns = [
    {
      ...
      replacement: '[REDACTED: ignore instructions]',
    },
    {
      pattern:
        /disregard\s+(?:previous|prior|above|all)\s+(?:instructions?|prompts?|rules?)/gi,
      replacement: '[REDACTED: disregard instructions]',
    },
    {
      pattern:
        /you\s+are\s+(?:now\s+)?(?:a\s+)?(?:hacker|admin|root|developer|system|god)/gi,
      replacement: '[REDACTED: role override]',
    },
    {
      pattern:
        /act\s+as\s+(?:a\s+)?(?:hacker|admin|root|developer|system|god)/gi,
      replacement: '[REDACTED: role override]',
    },
    {
      pattern:
        /show\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions?|rules?)/gi,
      replacement: '[REDACTED: prompt extraction]',
    },
    {
    ...
    },
    {
      ...
      replacement: '[REDACTED: developer mode]',
    },
  ];
  ...
  logDetection(
    ...
    if (result.severity === 'high') {
      this.logger.error(
        `HIGH SEVERITY INJECTION: ${JSON.stringify(result.matches)}`,
      );
    }
  }
