...
describe('tool-call-repair', () => {
  ...
  it('repairs <tool_call> XML tag', () => {
    const content =
      'Saya cek dulu <tool_call>{"name":"list_workspace_files","arguments":{"workspaceId":"w1"}}</tool_call>';
    const calls = repairToolCalls(content);
    expect(calls).toHaveLength(1);
  ...
  it('repairs <function_call name=...> attribute form', () => {
    const content =
      '<function_call name="edit_workspace_file">{"workspaceId":"w1","filename":"x.txt","instructions":"update"}</function_call>';
    const calls = repairToolCalls(content);
    expect(calls).toHaveLength(1);
  ...
  it('repairs bare JSON object', () => {
    const content =
      'Saya akan gunakan tool berikut: {"name": "calculate", "arguments": {"items": []}}';
    const calls = repairToolCalls(content);
    expect(calls).toHaveLength(1);
  ...
  it('repairs function wrapper object', () => {
    const content =
      '{"function": {"name": "search_workspace", "arguments": {"query": "rekap"}}}';
    const calls = repairToolCalls(content);
    expect(calls).toHaveLength(1);
  ...
  it('repairs multiple calls in one message', () => {
    const content =
      '<tool_call>{"name":"list_workspace_files","arguments":{}}</tool_call> <tool_call>{"name":"read_workspace_file","arguments":{"filePath":"a"}}</tool_call>';
    const calls = repairToolCalls(content);
  ...
  it('repairs tool_call: prefix', () => {
    const content =
      'tool_call: {"name": "delete_workspace_file", "arguments": {"filename": "b.txt"}}';
    const calls = repairToolCalls(content);
    expect(calls).toHaveLength(1);
  ...
  it('repairOneToolCall handles attribute + args', () => {
    const call = repairOneToolCall(
      'name="write_workspace_file" {"filename":"x.txt","content":"hi"}',
      0,
    );
