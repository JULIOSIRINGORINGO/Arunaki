...
const SECRET = {
  secret: 'some-client-secret-abcdef',
  iterations: 1000,
  keySize: 12,
};
...
describe('CryptoHarvesterService.learnFromCaptures', () => {
  it('registers a learned stock API from encrypted request + stock payload + secret', async () => {
    ...
    const ok = await svc.learnFromCaptures(
      ...
      'https://shop.example.com/p/kaos-premium-7200-99',
      {
        encrypted: [
          {
            url: '/api/userapi/category/stock/99/Medan?isWholesale=true',
            body: '{"encrypt":"..."}',
          },
        ],
        decrypted: [
  ...
  it('learns only once per host', async () => {
    ...
    expect(
      await svc.learnFromCaptures('shop.example.com', 'u', captures, SECRET),
    ).toBe(true);
    expect(
      await svc.learnFromCaptures('shop.example.com', 'u', captures, SECRET),
    ).toBe(false);
