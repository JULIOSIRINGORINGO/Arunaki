  ...
  Effect.gen(function* () {
    ...
    yield* tools
      .register({
        [name]: Tool.make({
          description:
            "Read a text file or supported image, page through a large UTF-8 text file by line offset, or list a directory page. Use this tool only when the user's request requires reading or inspecting workspace files or documents. Do not call this tool for greetings or casual conversation. Relative paths resolve from the current location; absolute paths inside it are accepted, while external absolute paths require external_directory approval.",
          input: Input,
          output: Output,
