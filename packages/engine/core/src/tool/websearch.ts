...
export class ConfigService extends Context.Service<ConfigService, Config>()("@arunaki/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.Arunaki_WEBSEARCH_PROVIDER === "exa" || process.env.Arunaki_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.Arunaki_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("Arunaki_EXPERIMENTAL") || truthy("Arunaki_ENABLE_EXA") || truthy("Arunaki_EXPERIMENTAL_EXA"),
    enableParallel: truthy("Arunaki_ENABLE_PARALLEL") || truthy("Arunaki_EXPERIMENTAL_PARALLEL"),
