...
// test/preload.ts pins Arunaki_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.Arunaki_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.Arunaki_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.Arunaki_MODELS_PATH = undefined
  Flag.Arunaki_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.Arunaki_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.Arunaki_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.Arunaki_DISABLE_MODELS_FETCH = false
        }),
