...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Arunaki-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["Arunaki_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { Arunaki_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("Arunaki_TEST_SHELL=ok")
      }),
    )
