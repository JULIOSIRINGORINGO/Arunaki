...
import { GoogleVertexPlugin } from "@arunaki/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.Arunaki, (provider) => {
          provider.api = {
            ...
            url: "https://Arunaki.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.Arunaki))
