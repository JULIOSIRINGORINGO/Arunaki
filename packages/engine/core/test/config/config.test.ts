...
import { Policy } from "@arunaki/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads Arunaki JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
  ...
  it.live("loads global, ancestor, and .Arunaki configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
        ...
        return Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".Arunaki")),
              AbsolutePath.make(path.join(directory, ".Arunaki")),
            ])
