...
import { define } from "@arunaki/plugin/v2/effect"
...
import { LocationServiceMap } from "@arunaki/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "arunaki.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "arunaki.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
