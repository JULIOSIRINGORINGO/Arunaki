...
import { Watcher } from "@arunaki/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    Arunaki_EXPERIMENTAL_FILEWATCHER: "true",
    Arunaki_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@arunaki.test`.cwd(tmp.path).quiet()
