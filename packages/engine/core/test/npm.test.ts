...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@arunaki/acme")).toBe("@arunaki/acme")
    expect(Npm.sanitize("@arunaki/acme@1.0.0")).toBe("@arunaki/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/Arunaki/acme.git"
    const expected = win ? "acme@git+https_//github.com/Arunaki/acme.git" : spec
