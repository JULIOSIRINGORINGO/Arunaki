...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@arunaki/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@arunaki/core/effect/app-node-builder"
import { errorData, errorMessage } from "@Arunaki-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@Arunaki-ai/tui/context/theme"
import { Global } from "@arunaki/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@arunaki/core/util/flock"
import { Flag } from "@arunaki/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@Arunaki-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@arunaki/core/v1/config/plugin"
import { createCommandShim } from "@Arunaki-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@Arunaki-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".Arunaki"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".Arunaki", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".Arunaki", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.Arunaki_PURE ? [] : pluginOrigins
    if (Flag.Arunaki_PURE && pluginOrigins.length) {
    }
