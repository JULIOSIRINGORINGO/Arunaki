...
export const DEPRECATED_PLUGIN_PACKAGES = ["Arunaki-openai-codex-auth", "Arunaki-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, ArunakiVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(ArunakiVersion) || semver.major(ArunakiVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.Arunaki
  if (typeof range !== "string") return
  if (!semver.satisfies(ArunakiVersion, range)) {
    throw new Error(`Plugin requires Arunaki ${range} but running ${ArunakiVersion}`)
  }
}
