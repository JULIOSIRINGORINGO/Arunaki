...
import { InstallationLocal, InstallationVersion } from "@arunaki/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@arunaki/core/v1/config/console-state"
import { FSUtil } from "@arunaki/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@arunaki/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@arunaki/core/v1/config/config"
import { RemoteAuthError } from "@arunaki/core/v1/config/error"
import { ConfigPermissionV1 } from "@arunaki/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@arunaki/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.Arunaki_CONFIG && !Flag.Arunaki_CONFIG_DIR && !Flag.Arunaki_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "Arunaki_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/Arunaki`
        ...
        if (Flag.Arunaki_CONFIG) {
          yield* merge(Flag.Arunaki_CONFIG, yield* loadFile(Flag.Arunaki_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.Arunaki_CONFIG })
        }

        if (!Flag.Arunaki_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("arunaki", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.Arunaki_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from Arunaki_CONFIG_DIR", { path: Flag.Arunaki_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.Arunaki/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.Arunaki_CONFIG_CONTENT) {
          const source = "Arunaki_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.Arunaki_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from Arunaki_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["Arunaki_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("Arunaki_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.Arunaki_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.Arunaki_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("Arunaki_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.Arunaki_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.Arunaki_DISABLE_PRUNE) {
