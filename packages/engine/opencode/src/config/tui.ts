...
import { isRecord } from "@Arunaki-ai/tui/util/record"
...
import { TuiKeybind } from "@Arunaki-ai/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@arunaki/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old Arunaki.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.Arunaki`
  // folders between cwd and home, and Arunaki_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.Arunaki_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit Arunaki_TUI_CONFIG override, if set.
  if (Flag.Arunaki_TUI_CONFIG) {
    const configFile = Flag.Arunaki_TUI_CONFIG
  ...
  // 4. `.Arunaki` directories (and Arunaki_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".Arunaki") || dir === Flag.Arunaki_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".Arunaki") && dir !== Flag.Arunaki_CONFIG_DIR) continue
