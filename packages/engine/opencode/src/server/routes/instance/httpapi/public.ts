...
import { ArunakiHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for Arunaki serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ArunakiHttpApi)
...
export const PublicApi = ArunakiHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "arunaki",
    version: "1.0.0",
    description: "Arunaki api",
