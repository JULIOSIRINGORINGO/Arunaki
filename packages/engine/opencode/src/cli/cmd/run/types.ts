// Shared type vocabulary for the direct interactive mode (`Arunaki --mini`).
//
...
import type { ArunakiClient, PermissionRequest, QuestionRequest, ToolPart } from "@arunaki/sdk/v2"
import type { TuiConfig } from "@Arunaki-ai/tui/config"
...
type PromptModel = Parameters<ArunakiClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ArunakiClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ArunakiClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ArunakiClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ArunakiClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ArunakiClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ArunakiClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ArunakiClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ArunakiClient["question"]["reject"]>[0]
