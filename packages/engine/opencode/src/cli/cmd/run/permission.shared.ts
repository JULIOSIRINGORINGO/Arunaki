...
import type { PermissionRequest } from "@arunaki/sdk/v2"
...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until Arunaki is restarted.`]
  }

  return [
    "This will allow the following patterns until Arunaki is restarted.",
