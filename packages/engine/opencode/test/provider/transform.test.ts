    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".Arunaki/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, ArunakiModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.Arunaki?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.Arunaki?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              Arunaki: { itemId: "msg_Arunaki_part" },
    ...
    expect(result[0].providerOptions?.Arunaki?.itemId).toBe("msg_Arunaki")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.Arunaki?.itemId).toBe("msg_Arunaki_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["arunaki", "deepseek-v4-flash"],
    ["Arunaki-go", "deepseek-v4-flash"],
