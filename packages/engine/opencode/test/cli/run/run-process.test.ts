// Subprocess integration tests for `Arunaki run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `Arunaki.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `Arunaki_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("Arunaki run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const result = yield* Arunaki.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const result = yield* Arunaki.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* Arunaki.run("think", { extraArgs: ["--thinking"] })
        Arunaki.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* Arunaki.run("think again")
        Arunaki.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const result = yield* Arunaki.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const result = yield* Arunaki.run("say hi", { format: "json" })
        Arunaki.expectExit(result, 0)

        const events = Arunaki.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const result = yield* Arunaki.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const result = yield* Arunaki.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* Arunaki.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* Arunaki.run("request permission", { permission: { bash: "ask" } })
        Arunaki.expectExit(denied, 0)
        ...
        const allowed = yield* Arunaki.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        Arunaki.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* Arunaki.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        Arunaki.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, Arunaki }) =>
      Effect.gen(function* () {
        ...
        const server = yield* Arunaki.serve()

        const result = yield* Arunaki.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, Arunaki }) =>
      Effect.gen(function* () {
        const result = yield* Arunaki.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, Arunaki }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* Arunaki.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
