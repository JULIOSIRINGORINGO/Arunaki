...
describe("Arunaki acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ Arunaki }))
        ...
        expect(initialized.agentInfo?.name).toBe("arunaki")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ Arunaki })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("Arunaki-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "Arunaki-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.Arunaki_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("Arunaki-login")
