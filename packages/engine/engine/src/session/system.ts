      ...
      environment: Effect.fn("SystemPrompt.environment")(function* (model: Provider.Model) {
        ...
        const envLines = isScratch
          ...
          : [
              ...
              `  * For casual greetings, quotes, general knowledge, creative prompts, jokes, or conversational follow-up questions (e.g. "kata kata hari ini", "ada yang menarik?", "ada lagi?", "cerita dong", "rekomendasi", "siapa kamu", "bisa bantu apa"), respond conversationally and naturally WITHOUT executing any tools (do NOT read files, do NOT list directories, do NOT execute shell commands).`,
              `  * NEVER assume ambiguous or casual questions like "ada yang menarik?" mean inspecting the user's workspace documents. Answer within the conversational context (e.g. provide interesting quotes, trivia, or thoughts).`,
              `  * Tools must ONLY be executed when the user explicitly requests an operation on documents/files or clearly provides unformatted raw data/notes to be recorded (e.g. "rekap ini ke excel", "baca file laporan", "update catatan ini", or pastes sales/order data).`,
              `- SCRATCH SCRIPT SAFETY & POLITE MESSAGING (STRICT):`,
              `  * If executing Python scripts in scratch directory, NEVER name helper scripts after Python standard library modules (e.g. DO NOT name scripts inspect.py, test.py, string.py, code.py, json.py, io.py). Always prefix with 'arunaki_' (e.g. 'arunaki_calc.py').`,
              `  * NEVER dump internal script debugging, file collision details, or verbose step-by-step programming narrations (such as "Nama file-nya bentrok dengan modul bawaan...") into the user chat. Keep communications clean, focused, and user-friendly.`,
