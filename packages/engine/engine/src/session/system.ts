      ...
      environment: Effect.fn("SystemPrompt.environment")(function* (model: Provider.Model) {
        ...
        const envLines = isScratch
          ...
          : [
              ...
              `- DOCUMENT & TOOL PRIORITY (NATIVE FIRST, SCRIPT FALLBACK):`,
              `  * Priority 1 (Native Tools): ALWAYS try Arunaki's native built-in tools first:`,
              `    - Word documents (.docx): Use 'word_read' to instantly extract text, paragraphs, and tables (<100ms, zero python needed).`,
              `    - Excel spreadsheets (.xlsx, .xls, .csv): Use 'excel_read' to inspect sheets, cells, formulas, and values.`,
              `    - PowerPoint presentations (.pptx): Use 'ppt_read' to inspect slides and shapes.`,
              `    - Text/Markdown/JSON/Code: Use 'read'.`,
              `  * Priority 2 (Fallback Scripts): If a native tool fails (e.g. damaged or unsupported document structure) or for advanced mathematical calculations, you may write helper scripts in .arunaki/scratch/. Do NOT default to custom scripts or package installs when native tools can read the document directly.`,
