...
import type { LLMClientShape } from "@arunaki/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("arunaki"))
    return { type: "unsupported", reason: "provider is not openai, Arunaki, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @arunaki/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains Arunaki-owned. The native runtime only adapts
      // the @arunaki/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
