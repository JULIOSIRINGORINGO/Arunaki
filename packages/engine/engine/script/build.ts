...
const createEmbeddedWebUIBundle = async () => {
  ...
  await $`Arunaki_CHANNEL=${Script.channel} bun run --cwd ${appDir} build`
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/Arunaki`,
      execArgv: [`--user-agent=Arunaki/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: {
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/Arunaki`
