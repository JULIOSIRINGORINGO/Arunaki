// Subprocess test harness for the Arunaki CLI. Spawns the real binary against
...
//
// Today only `Arunaki.run` is fully wired. The shape supports adding more
// builders (`Arunaki.serve(opts)`, `Arunaki.acp(opts)`, `Arunaki.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ArunakiCli.
...
const ArunakiRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(ArunakiRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    Arunaki_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    Arunaki_CONFIG_CONTENT: configJson,
    Arunaki_DISABLE_PROJECT_CONFIG: "1",
    Arunaki_PURE: "1",
    Arunaki_DISABLE_AUTOUPDATE: "1",
    Arunaki_DISABLE_AUTOCOMPACT: "1",
    Arunaki_DISABLE_MODELS_FETCH: "1",
    Arunaki_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `Arunaki run`. New flags should
...
// `Arunaki serve` is a long-lived process — it never exits on its own.
...
// `Arunaki acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ArunakiCli = {
  ...
  // Spawn `Arunaki serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `Arunaki acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("Arunaki.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          Arunaki_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("Arunaki.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("Arunaki.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "Arunaki server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `Arunaki serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("Arunaki.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const Arunaki: ArunakiCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, Arunaki })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `Arunaki.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "arunaki") {
...
// every call site. Use this for any test that needs the Arunaki CLI fixture.
//
...
// resources (e.g. `Arunaki.serve`) without an extra `Effect.scoped` wrapper —
