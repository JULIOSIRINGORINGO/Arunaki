...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".Arunaki")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .Arunaki/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const Arunaki = path.join(test.directory, ".Arunaki")
      const tool = path.join(Arunaki, "tool")
  ...
  it.instance("ignores non-tool exports in .Arunaki/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".Arunaki", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-Arunaki` was registering a tool
  ...
  it.instance("loads tools from .Arunaki/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const Arunaki = path.join(test.directory, ".Arunaki")
      const tools = path.join(Arunaki, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".Arunaki", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const Arunaki = path.join(test.directory, ".Arunaki")
        const customTools = path.join(Arunaki, "tools")
        const plugin = path.join(Arunaki, "node_modules", "@Arunaki-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(Arunaki, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@arunaki/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@arunaki/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".Arunaki", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(Arunaki, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@arunaki/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(Arunaki, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(Arunaki, "node_modules", "cowsay")
