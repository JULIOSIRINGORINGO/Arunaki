...
// terminal sizes. The default Arunaki tui command is excluded —
// `Arunaki --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `Arunaki --help` prints today;
...
// top-level help on `--help` and exits 1; not a real Arunaki command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("Arunaki CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const topLevel = yield* Arunaki.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* Arunaki.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`Arunaki ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`Arunaki ${argv.join(" ")} --help`)
        }
