...
describe("Arunaki read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // Arunaki_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["mcp", "list"])
        Arunaki.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["providers", "list"])
        Arunaki.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["models"])
        Arunaki.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["agent", "list"])
        Arunaki.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh Arunaki_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["session", "list"])
        Arunaki.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["stats"])
        Arunaki.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under Arunaki_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ Arunaki }) =>
      Effect.gen(function* () {
        const r = yield* Arunaki.spawn(["db", "path"])
        Arunaki.expectExit(r, 0, "db path")
