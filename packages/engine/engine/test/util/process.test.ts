...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.Arunaki_TEST ?? "")'), {
      env: {
        Arunaki_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "Arunaki_TEST_SHELL"], {
      shell: true,
      env: {
        Arunaki_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("Arunaki_TEST_SHELL=ok")
  })
