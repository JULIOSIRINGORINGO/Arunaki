...
import { PromptInput } from "@arunaki/schema/prompt-input"
import { Session } from "@arunaki/schema/session"
import { Project } from "@arunaki/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@arunaki/schema/schema"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Arunaki process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Arunaki process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
